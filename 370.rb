...
#   what the check digit must be for a valid code. (Check digits have previously appeared 
#     in this subreddit: see Intermediate 30 and Easy 197.) UPC's check digit is calculated 
#     as follows (taken from Wikipedia):
# 
...
#       Take the sum of digits at even-numbered positions (2nd, 4th, 6th, ..., 10th) in the 
#       original number, and add this sum to the result from step 2.
...
# 
# Challenge
# Given an 11-digit number, find the 12th digit that would make a valid UPC. You may treat the 
# input as a string if you prefer, whatever is more convenient. If you treat it as a number, 
# you may need to consider the case of leading 0's to get up to 11 digits. That is, an input of 
#   12345 would correspond to a UPC start of 00000012345.
# 
# Examples
# upc(4210000526) => 4
# upc(3600029145) => 2
# upc(12345678910) => 4
# upc(1234567) => 0
# Also, if you live in a country that uses UPCs, you can generate all the examples you want by 
# picking up store-bought items or packages around your house. Find anything with a bar code on 
